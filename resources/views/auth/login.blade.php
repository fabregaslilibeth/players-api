...
@section('content')
    <div class="h-128 flex items-center bg-white border-none">
        <div class="top-20 relative rounded-lg w-11/12 md:w-9/12 lg:w-7/12 mx-auto block md:flex items-center bg-white shadow-xl"
            ...
            <div class="relative md:w-2/5 h-full overflow-hidden rounded-t-lg md:rounded-t-none md:rounded-l-lg"
                ...
                <div class="absolute inset-0 w-full h-full flex items-center justify-center fill-current text-white">
                    <h1 class="text-3xl tracking-widest">LOGIN</h1>
                </div>
            </div>
                ...
                <div class="mx-auto md:pr-24 md:pl-16">
                    <form method="POST" action="{{ route('login') }}" autocomplete="off">
                        ...
                        <div class="my-4">
                            ...
                            <label class="text-xs underline text-primary" for="remember">
                                {{ __('Remember Me') }}
                            </label>
                        </div>
                        ...
                        <div class="my-4">
                            @if (Route::has('password.request'))
                                <a class="text-primary text-xs underline" href="{{ route('password.request') }}">
                        ...
                        <div class="my-4">
                            @if (Route::has('register'))
                                <span class="text-xs">Not signed in yet?</span>
                                <a class="text-primary text-xs underline" href="{{ route('register') }}">
                                    {{ __('Register') }}
                                </a>
                            @endif
                        </div>
